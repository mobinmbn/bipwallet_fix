...
class PaymentChannelModel:
    ...
    def __repr__(self):
        return "<Channel(url='{}', state='{}', creation_time={}, deposit_tx='{}', refund_tx='{}', payment_tx='{}', spend_tx='{}', spend_txid='{}', min_output_amount={})>".format(
            self.url, self.state, self.creation_time, self.deposit_tx, self.refund_tx, self.payment_tx, self.spend_tx,
            self.spend_txid, self.min_output_amount)  # nopep8
...
class PaymentChannelRedeemScript(bitcoin.Script):
    ...
    def __init__(self, merchant_public_key, customer_public_key, expiration_time):
        ...
        """
        super().__init__(["OP_IF", merchant_public_key.compressed_bytes, "OP_CHECKSIGVERIFY", "OP_ELSE",
                          expiration_time.to_bytes(math.ceil(expiration_time.bit_length() / 8), 'little'),
                          "OP_CHECKLOCKTIMEVERIFY", "OP_DROP", "OP_ENDIF", customer_public_key.compressed_bytes,
                          "OP_CHECKSIG"])  # nopep8
    ...
    def from_bytes(cls, b):
        ...
        if not bitcoin.Script.validate_template(self, ["OP_IF", bytes, "OP_CHECKSIGVERIFY", "OP_ELSE", bytes,
                                                       "OP_CHECKLOCKTIMEVERIFY", "OP_DROP", "OP_ENDIF", bytes,
                                                       "OP_CHECKSIG"]):  # nopep8
...
class PaymentChannelStateMachine:
    ...
    def create(self, merchant_public_key, deposit_amount, expiration_time, fee_amount, zeroconf, use_unconfirmed=False):
        ...
        # Build deposit tx
        try:
            deposit_tx = self._wallet.create_deposit_tx(redeem_script.address(),
                                                        deposit_amount + PaymentChannelStateMachine.PAYMENT_TX_MIN_OUTPUT_AMOUNT,
                                                        fee_amount, use_unconfirmed=use_unconfirmed)  # nopep8
    ...
    def deposit_amount(self):
        ...
        """
        return (self._model.refund_tx.outputs[
                    0].value - self._model.min_output_amount) if self._model.refund_tx else None  # nopep8
    ...
    def fee_amount(self):
        ...
        """
        return (self._model.deposit_tx.outputs[self.deposit_tx_utxo_index].value - self._model.refund_tx.outputs[
            0].value) if self._model.refund_tx else None  # nopep8
    ...
    def deposit_tx_utxo_index(self):
        ...
        """
        return self._model.deposit_tx.output_index_for_address(
            self._redeem_script.hash160()) if self._model.deposit_tx else None  # nopep8
    ...
    def deposit_txid_signature(self):
        ...
        """
        return codecs.encode(self._wallet.sign(self.deposit_txid.encode('ascii'), self._customer_public_key).to_der(),
                             'hex_codec').decode() if self._model.deposit_tx else None  # nopep8
